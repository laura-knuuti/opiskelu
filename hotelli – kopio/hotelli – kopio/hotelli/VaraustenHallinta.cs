...
namespace hotelli
{
    public partial class VaraustenHallinta : Form
    {
        ...
        private void varausBT_Click(object sender, EventArgs e)
        {
            ...
            //korjaillaan päivämääräristiriitoja
            if (sisaankirjautuminen.Date < DateTime.Today)
            {
                MessageBox.Show("Sisäänkirjautumispäivä ei voi olla menneisyydessä.", "Päivämäärävirhe", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            if (uloskirjautuminen <= sisaankirjautuminen)
            {
                MessageBox.Show("Uloskirjautumispäivän täytyy olla myöhempi kuin sisäänkirjautumispäivä.", "Päivämäärävirhe", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            //end
        ...
        {
            if (huonenroCB.SelectedValue == null)
            {
                MessageBox.Show("Valitse huonenumero ennen muokkausta", "Virhe", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            if (asiakasCB.SelectedValue == null)
            {
                MessageBox.Show("Valitse asiakas ennen muokkausta", "Virhe", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            if (string.IsNullOrEmpty(varausnroTB.Text))
            {
                MessageBox.Show("Valitse varaus ennen muokkausta", "Virhe", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void poistaBT_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!int.TryParse(varausnroTB.Text, out int varausnro)) 
                {
                    MessageBox.Show("Anna kelvollinen varaustunnus", "Virhe", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                if (huonenroCB.SelectedValue == null)
                {
                    MessageBox.Show("Valitse huonenumero", "Virhe", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                int huonenro = Convert.ToInt32(huonenroCB.SelectedValue);
        ...
        private void ulosDTP_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (huonenroCB.SelectedValue == null)
            {
                MessageBox.Show("Valitse huonenumero ennen jatkamista", "Virhe", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            int huonenro = Convert.ToInt32(huonenroCB.SelectedValue.ToString());
            DateTime sisaankirjautuminen = Convert.ToDateTime(sisaanDTP.Value);
